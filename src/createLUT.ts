...
async function processWSOLInstructionsATA(
  jitoTipAmt: number,
  blockhash: string | Blockhash
): Promise<VersionedTransaction[]> {
  ...
  for (let i = 0; i < instructionChunks.length; i++) {
    ...
    const versionedTx = await createAndSignVersionedTxNOLUT(
      instructionChunks[i],
      blockhash
    );
