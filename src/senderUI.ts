...
async function generateSOLTransferForKeypairs(
  ...
  keypairs.forEach((keypair, index) => {
    ...
    console.log(
      `Transfer of ${Number(amount) / LAMPORTS_PER_SOL} SOL to Wallet ${
        index + 1
      } (${keypair.publicKey.toString()}) bundled.`
    );
  });
}
...
async function generateATAForKeypairs(baseAddr: string, steps: number = 27) {
  ...
  for (const [index, keypair] of keypairs.entries()) {
    ...
    console.log(
      `Created ATA for Wallet ${index + 1} (${keypair.publicKey.toString()}).`
    );
  }
...
async function processInstructionsATA(
  jitoTipAmt: number,
...
async function distributeWSOL(
  distributeAmt: number,
  jitoTip: number,
  steps = 27
) {
  const keypairs = loadKeypairs();
  const totalDistributedAmount: number =
    (distributeAmt * steps * (steps + 1)) / 2; // Sum of arithmetic series formula
  ...
  for (let i = 0; i < Math.min(steps, keypairs.length); i++) {
    ...
    console.log(
      `Distributed ${(incrementalAmount / LAMPORTS_PER_SOL).toFixed(
        2
      )} WSOL to Wallet ${i + 1} (${keypair.publicKey.toString()}) ATA`
    );
  }
...
async function sendBundle() {
  try {
    const bundleId = await searcherClient.sendBundle(
      new JitoBundle(sendTxns, sendTxns.length)
    );
  ...
  while (running) {
    ...
    switch (answer) {
      ...
      case "5":
        ...
        break;
      case "6":
        const initialBuyAmt = prompt(
          "Distribute amount increment (Ex: 0.05): "
        );
