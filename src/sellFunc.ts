...
export async function sellXPercentage() {
  ...
  const inputPercentageOfSupply =
    prompt("Percentage to sell (Ex. 1 for 1%): ") || "1";
  const jitoTipAmtInput = prompt("Jito tip in Sol (Ex. 0.01): ") || "0";
  const jitoTipAmt = parseFloat(jitoTipAmtInput) * LAMPORTS_PER_SOL;
  ...
  for (let chunk of chunkedKeypairs) {
    ...
    for (let keypair of chunk) {
      const tokenBalanceRaw = await fetchTokenBalance(
  ...
  const destination = await spl.getAssociatedTokenAddress(
    spl.NATIVE_MINT,
...
function makeSell(
  ...
  const programId = new PublicKey(
    "47MBhTFFxKaswZioPvRMqRqVYTd668wZiGc3oKLZP2tx"
  ); // MY PROGRAM
